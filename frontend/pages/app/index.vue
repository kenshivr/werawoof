...
const matchResult = ref<{ matched: boolean; matchId?: string } | null>(null)
...
async function performSwipe(direction: 'like' | 'dislike') {
  ...
  if (result.status === 'fulfilled' && result.value?.match) {
    matchResult.value = { matched: true, matchId: result.value.match.id }
    setTimeout(() => (matchResult.value = null), 3000)
  }
...
onMounted(async () => {
  ...
  if (dogsStore.dogs.length > 0) {
    activeDog.value = dogsStore.dogs[0]
...
<template>
  <div class="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] px-4 py-8">
    ...
    <Transition name="fade">
      <div
        v-if="matchResult"
        class="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-[#F4C07D] text-[#382615] px-8 py-4 rounded-2xl shadow-xl font-h3 text-h3 font-jakarta flex items-center gap-3"
      >
    ...
    <!-- No dog state -->
    <div v-if="!loadingCandidates && dogsStore.dogs.length === 0" class="text-center space-y-4">
      <span class="material-symbols-outlined text-6xl text-[#382615]/30">pets</span>
      <h2 class="text-h3 font-h3 text-on-surface font-jakarta">Agregá un perro para empezar</h2>
      <p class="text-body-md text-on-surface-variant">
        Necesitás al menos un perfil de perro para encontrar compañeros.
      </p>
      <NuxtLink
        to="/app/dogs/new"
        class="inline-block bg-[#F4C07D] text-[#382615] px-8 py-4 rounded-2xl font-jakarta font-bold hover:-translate-y-0.5 transition-all duration-200 shadow-lg"
      >
        Agregar mi perro
      </NuxtLink>
    </div>
    ...
    <!-- Loading state -->
    <div v-else-if="loadingCandidates && candidates.length === 0" class="text-center space-y-3">
    ...
    <template v-else>
      <!-- Card stack -->
      <div class="relative w-full max-w-[480px]" style="height: 580px">
        <!-- Stack card 3 (bottom) -->
        <div
      ...
      <div class="mt-8 flex items-center gap-8">
        <!-- Dislike -->
        <button
          class="w-16 h-16 rounded-full bg-white flex items-center justify-center text-stone-400 shadow-[0_4px_20px_rgba(113,62,24,0.08)] hover:bg-stone-100 active:scale-90 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <!-- Super like -->
        <button
          class="w-12 h-12 rounded-full bg-white flex items-center justify-center text-tertiary shadow-[0_4px_20px_rgba(113,62,24,0.08)] hover:bg-tertiary-fixed/20 active:scale-90 transition-all duration-200"
        >
        ...
        <!-- Like -->
        <button
          class="w-20 h-20 rounded-full bg-[#F4C07D] flex items-center justify-center text-[#382615] shadow-[0_4px_20px_rgba(113,62,24,0.12)] hover:opacity-90 active:scale-90 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
          :disabled="isAnimating"
      ...
      <div class="mt-6 flex gap-3 flex-wrap justify-center">
        <div
          class="bg-white/60 backdrop-blur-sm px-4 py-2 rounded-full flex items-center gap-2 text-on-surface-variant text-label-md font-label-md shadow-sm"
        >
          <span class="material-symbols-outlined text-sm">tune</span>
          <span>Cerca · Todas las razas</span>
        </div>
      </div>
    </template>
    ...
    <Transition name="profile-fade">
      <div v-if="showProfile && topCandidate" class="fixed inset-0 z-[100]">
        ...
        <!-- Mobile action buttons — absolute del fixed parent, por encima del bottom nav -->
        <div
                ...
                <div v-if="topCandidate.distance" class="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <span class="font-bold text-[#281808] mt-1"
                      >{{ topCandidate.distance.toFixed(1) }} km de distancia</span
                    >
                  </div>
                </div>
