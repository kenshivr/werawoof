...
<template>
  <div class="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] px-4 py-8">
    ...
    >
      <div class="flex items-center gap-2 bg-white/90 backdrop-blur-md border border-[#fdddc3] px-4 py-2 rounded-full shadow-sm text-sm font-medium text-[#382615] font-jakarta pointer-events-auto">
        <div class="w-6 h-6 rounded-full overflow-hidden bg-[#fff1e8] shrink-0">
          ...
          />
          <span v-else class="material-symbols-outlined text-sm text-[#F4C07D] flex items-center justify-center w-full h-full">pets</span>
        </div>
        <span>Explorando como <strong>{{ activeDog.name }}</strong></span>
    ...
    <template v-else>
      ...
      <div v-else-if="noMoreCandidates && candidates.length === 0" class="text-center space-y-4">
        <span class="material-symbols-outlined text-6xl text-[#382615]/30">sentiment_satisfied</span>
        <h2 class="text-h3 font-h3 text-on-surface font-jakarta">¡Ya viste a todos los de tu zona!</h2>
        <p class="text-body-md text-on-surface-variant">Volvé más tarde para ver nuevos peludos cerca.</p>
        <NuxtLink
          to="/app/dogs"
          class="inline-flex items-center gap-2 text-[#795832] font-medium hover:text-[#382615] transition-colors"
        >
          <span class="material-symbols-outlined text-base">arrow_back</span>
          Explorar con otro perro
        </NuxtLink>
      </div>
              ...
              <div class="flex items-end justify-between gap-4">
                <div class="flex-1 min-w-0 pointer-events-none">
                  <div class="flex items-center gap-2 mb-2">
                    <h2 class="text-3xl font-extrabold font-jakarta">{{ topCandidate.name }}, {{ topCandidate.age }}</h2>
                    <span class="material-symbols-outlined text-[#F4C07D]" style="font-variation-settings: 'FILL' 1">verified</span>
                  </div>
                  <div class="flex flex-wrap gap-2 mb-3">
                    <span class="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider border border-white/20">{{ topCandidate.breed }}</span>
                    <span v-if="topCandidate.distance" class="bg-black/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                      <span class="material-symbols-outlined text-sm">location_on</span>
                      {{ topCandidate.distance.toFixed(1) }} km
                    </span>
                  </div>
                  <p v-if="topCandidate.bio" class="text-white/90 text-sm line-clamp-2">{{ topCandidate.bio }}</p>
                </div>
                <button
    ...
    <Transition name="profile-fade">
      <div v-if="showProfile && topCandidate" class="fixed inset-0 z-[100]">
        ...
        <div class="md:hidden absolute inset-0 bg-white overflow-y-auto no-scrollbar">
          <nav class="fixed top-0 left-0 right-0 z-10 flex justify-between items-center px-6 py-6 pointer-events-none">
            <button
          ...
          <section class="relative h-[530px] w-full bg-stone-200">
            <img :src="topCandidate.photos?.[profilePhotoIdx] ?? dogPhoto(topCandidate)" :alt="topCandidate.name" class="w-full h-full object-cover" />
            <div class="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent pointer-events-none" />
            <button v-if="(topCandidate.photos?.length ?? 0) > 1" class="absolute inset-y-0 left-0 w-1/2" @click="prevPhoto" />
            <button v-if="(topCandidate.photos?.length ?? 0) > 1" class="absolute inset-y-0 right-0 w-1/2" @click="nextPhoto" />
            <div v-if="(topCandidate.photos?.length ?? 0) > 1" class="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-1.5 px-3 py-1.5 bg-black/20 backdrop-blur-md rounded-full">
              <div v-for="(_, i) in topCandidate.photos" :key="i" class="w-2 h-2 rounded-full transition-all duration-200" :class="i === profilePhotoIdx ? 'bg-white' : 'bg-white/40'" />
            </div>
          </section>
          <section class="relative -mt-8 bg-white rounded-t-[32px] px-6 pt-8 pb-36">
            <div class="flex justify-between items-start mb-4">
              <div>
                <h1 class="font-jakarta font-extrabold text-[32px] text-[#281808] leading-tight flex items-center gap-2">
                  {{ topCandidate.name }}
                  <span class="material-symbols-outlined text-[#F4C07D] text-2xl" style="font-variation-settings: 'FILL' 1">verified</span>
                </h1>
                <p v-if="topCandidate.distance" class="text-[#4f4539] flex items-center gap-1 mt-1 text-sm">
                  <span class="material-symbols-outlined text-[18px]">location_on</span>
                  {{ topCandidate.distance.toFixed(1) }} km de distancia
                </p>
              </div>
              <div class="bg-[#fff1e8] px-4 py-2 rounded-2xl flex flex-col items-center shrink-0 ml-4">
                <span class="font-jakarta font-bold text-[#7d571e] text-xl leading-tight">{{ topCandidate.age }}</span>
                <span class="text-[10px] font-bold uppercase tracking-widest text-[#4f4539]">Años</span>
              </div>
            </div>
            <div class="flex flex-wrap gap-2 mb-7">
              <span class="bg-[#B78F64]/10 text-[#B78F64] px-4 py-2 rounded-full text-sm font-semibold">{{ topCandidate.breed }}</span>
            </div>
            <div v-if="topCandidate.bio" class="mb-8">
              <h3 class="font-jakarta font-bold text-xl text-[#281808] mb-3">Sobre {{ topCandidate.name }}</h3>
              <p class="text-[#4f4539] leading-relaxed">{{ topCandidate.bio }}</p>
            </div>
            <div v-if="(topCandidate.photos?.length ?? 0) > 1" class="mb-4">
              <p class="text-xs font-bold uppercase tracking-widest text-[#4f4539]/60 mb-3">Fotos</p>
              <div class="grid grid-cols-3 gap-2">
                <div
                  ...
                  :class="i === profilePhotoIdx ? 'ring-2 ring-[#F4C07D] ring-offset-2' : 'opacity-75 hover:opacity-100'"
                  @click="profilePhotoIdx = i"
                >
                  <img :src="photo" :alt="`${topCandidate.name} ${i + 1}`" class="w-full h-full object-cover" />
                </div>
              </div>
            </div>
          </section>
        ...
        <div class="md:hidden absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white to-transparent">
          <div class="max-w-md mx-auto flex gap-4">
            <button
              ...
              @click="closeProfile(); performSwipe('dislike')"
            >
            ...
            <button
              ...
              @click="closeProfile(); performSwipe('like')"
            >
        ...
        <div class="hidden md:flex absolute inset-0 items-center justify-center px-6">
          ...
          <div class="relative w-full max-w-4xl bg-white rounded-[16px] shadow-[0_12px_40px_rgba(113,62,24,0.12)] overflow-hidden flex" style="max-height: 88vh">
            <button
            ...
            <div class="w-1/2 relative bg-stone-100 min-h-[500px]">
              <img :src="topCandidate.photos?.[profilePhotoIdx] ?? dogPhoto(topCandidate)" :alt="topCandidate.name" class="absolute inset-0 w-full h-full object-cover" />
              <button v-if="(topCandidate.photos?.length ?? 0) > 1" class="absolute inset-y-0 left-0 w-1/2 z-10" @click="prevPhoto" />
              <button v-if="(topCandidate.photos?.length ?? 0) > 1" class="absolute inset-y-0 right-0 w-1/2 z-10" @click="nextPhoto" />
              <div v-if="(topCandidate.photos?.length ?? 0) > 1" class="absolute bottom-6 left-0 right-0 z-20 flex justify-center gap-2">
                <div
            ...
            <div class="w-1/2 flex flex-col overflow-y-auto custom-scrollbar">
              <div class="flex-1 p-8 pb-4 space-y-6">
                <div>
                  <h1 class="font-jakarta font-extrabold text-[32px] text-[#281808] leading-tight mb-1">{{ topCandidate.name }}, {{ topCandidate.age }}</h1>
                ...
                <div v-if="topCandidate.distance" class="grid grid-cols-2 gap-4">
                  <div class="bg-[#fff1e8] p-4 rounded-[16px] flex flex-col items-center col-span-2">
                    <span class="material-symbols-outlined text-[#795832] mb-1">location_on</span>
                    <span class="text-xs font-bold uppercase tracking-wide text-[#4f4539]">Distancia</span>
                    <span class="font-bold text-[#281808] mt-1">{{ topCandidate.distance.toFixed(1) }} km</span>
                  </div>
                </div>
              ...
              <div class="flex-shrink-0 flex gap-4 px-8 py-5 bg-white/90 backdrop-blur-md border-t border-[#fdddc3] sticky bottom-0">
                <button
