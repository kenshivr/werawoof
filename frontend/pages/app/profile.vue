    ...
    <div class="hidden md:block max-w-[1280px] mx-auto px-6 py-16">
      ...
      <div class="w-full max-w-2xl mx-auto mb-10">
        <div class="flex justify-between items-end mb-3">
          <div>
            <span
              class="text-[#7d571e] font-bold text-xs uppercase tracking-widest block mb-1 font-jakarta"
            >
        ...
        <div class="w-full h-2 bg-[#ffeadb] rounded-full overflow-hidden">
          <div
            :style="{ width: hasDogs || currentStep === 2 ? '100%' : '50%' }"
            class="h-full bg-[#F4C07D] rounded-full transition-all duration-500"
          />
        </div>
      </div>
      ...
      <template v-if="currentStep === 1">
        <div
          class="bg-white rounded-3xl shadow-[0_4px_20px_rgba(113,62,24,0.08)] w-full max-w-2xl mx-auto overflow-hidden"
        >
          <div class="p-12">
            <form class="space-y-10" @submit.prevent="handleSaveStep1">
              ...
              <div class="flex flex-col items-center gap-5">
                <label class="relative cursor-pointer">
                  ...
                  <div
                    class="w-32 h-32 rounded-full bg-[#fff1e8] flex items-center justify-center border-2 border-dashed border-[#d3c4b4] overflow-hidden hover:border-[#F4C07D] transition-colors"
                  >
                    <img
                      v-if="avatarPreview"
                      :src="avatarPreview"
                  ...
                  <div
                    class="absolute -bottom-2 -right-2 bg-[#f4c07d] w-8 h-8 rounded-full shadow-md text-[#382615] flex items-center justify-center pointer-events-none"
                  >
                ...
                <div class="text-center">
                  <p
                    class="text-xs font-bold uppercase tracking-widest text-[#281808] font-jakarta"
                  >
              ...
              <div class="space-y-5">
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <div class="relative">
                    ...
                    />
                    <span
                      class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#d3c4b4]"
                      >location_on</span
                    >
                  </div>
                </div>
        ...
        <div class="mt-10 text-center text-[#4f4539] max-w-md mx-auto">
          <p class="text-sm italic">
            "Usamos tu ubicación para encontrar los mejores matches cerca tuyo. Podés cambiarlo
      ...
      <template v-if="currentStep === 2">
        <div class="grid grid-cols-12 gap-8 max-w-[1000px] mx-auto">
          ...
          <div class="col-span-7 space-y-6">
            <!-- Vital Statistics -->
            <div
              class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(113,62,24,0.08)] p-8 space-y-6"
            >
              <h2
              ...
              <div class="flex flex-col gap-3">
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-[#7d571e] font-bold font-jakarta"
                    >{{ dogForm.age }} {{ dogForm.age === 1 ? 'año' : 'años' }}</span
                  >
                </div>
                <input
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  <div class="flex p-1 bg-[#fff1e8] rounded-xl w-full">
                    <button
                      ...
                      :class="
                        dogForm.sex === 'male'
                          ? 'bg-white text-[#7d571e] shadow-sm'
                          : 'text-[#4f4539]'
                      "
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-wrap gap-2">
                  <button
                    ...
                    :class="
                      dogForm.personality_tags.includes(tag)
                        ? 'bg-[#B78F64]/10 border-[#B78F64] text-[#B78F64] font-bold'
    ...
    <div class="md:hidden relative min-h-screen">
      ...
      <div class="px-6 pt-6 pb-4 flex flex-col items-center gap-4">
        <div class="w-full max-w-md bg-[#382615]/10 h-1.5 rounded-full overflow-hidden">
          <div
            :style="{ width: hasDogs || currentStep === 2 ? '100%' : '50%' }"
            class="bg-[#F4C07D] h-full rounded-full transition-all duration-500"
          />
        </div>
        <div class="w-full flex justify-between px-1">
          <span class="text-xs font-bold uppercase tracking-widest text-[#795832] font-jakarta"
            >Paso {{ currentStep }} de {{ hasDogs ? 1 : 2 }}</span
          >
          <span
            class="text-xs font-bold uppercase tracking-widest text-[#795832] font-jakarta flex items-center gap-1"
          >
      ...
      <template v-if="currentStep === 1">
        <div class="px-6 pb-40">
          <section class="mb-7">
            ...
            <p class="text-base text-[#382615]/70">
              Dejá que la manada sepa quién maneja la correa.
            </p>
          </section>
          ...
          <form class="flex flex-col gap-6" @submit.prevent="handleSaveStep1">
            ...
            <section class="flex flex-col items-center">
              <label class="relative cursor-pointer">
                <input type="file" accept="image/*" class="sr-only" @change="handleFileChange" />
                <div
                  class="w-32 h-32 rounded-full bg-white shadow-[0_4px_20px_rgba(113,62,24,0.08)] border-4 border-white overflow-hidden flex items-center justify-center"
                >
                  <img
                    v-if="avatarPreview"
                    :src="avatarPreview"
                ...
                <div
                  class="absolute bottom-0 right-0 bg-[#f4c07d] w-9 h-9 rounded-full shadow-lg border-2 border-white flex items-center justify-center pointer-events-none"
                >
                  <span class="material-symbols-outlined text-[#382615] text-lg leading-none"
                    >photo_camera</span
                  >
                </div>
              </label>
              <p
                class="text-xs font-bold uppercase tracking-widest text-[#795832] mt-4 font-jakarta"
              >
                Sube tu foto de perfil
              </p>
            </section>
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <div class="flex p-1 bg-[#fff1e8] rounded-xl">
              <button
                ...
                :class="
                  dogForm.sex === 'male'
                    ? 'bg-[#F4C07D] text-[#382615] shadow-sm'
                    : 'text-[#4f4539]'
                "
                @click="dogForm.sex = 'male'"
              >
...
const handleSaveStep1 = async () => {
  ...
  saving.value = true
  try {
    // Garantizar datos frescos antes de decidir el flujo
